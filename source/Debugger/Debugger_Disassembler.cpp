...
//	char *sImmediate_, char & nImmediate_, char *sBranch_ )
{
	...
	{  // Regular 6502/65C02 opcode -> mnemonic
...
void FAC_Unpack (WORD nAddress, FAC_t &fac_)
{
	...
	fac_.mantissa = 0 | ((m1 | 0x80) << 24)  // implicit 1.0, EB12: ORA #$80, STA FAC+1
	                | ((m2) << 16) | ((m3) << 8) | ((m4) << 0);
...
std::string FormatDisassemblyLine (const DisasmLine_t &line)
{
	...
	std::string sDisassembly = StrFormat("%s:%s %s ", line.sAddress, line.sOpCodes, g_aOpcodes[ line.iOpcode ].sMnemonic);
