...
{
	DebugBreakOnDMA() :
	    isToOrFromMemory(0), memoryAddr(0), memoryAddrEnd(0), BPid(0) {}
...
struct ProfileLine_t
{
	...
	ProfileLine_t(char *_buf, size_t _bufsz) :
	    buf(_buf), bufsz(_bufsz) {}
...
Update_t CmdBookmarkSave (int nArgs)
{
	...
	while (iBookmark < MAX_BOOKMARKS)
	{
		if (g_aBookmarks[ iBookmark ].bSet)
		{
			g_ConfigState.PushLineFormat("%s %x %04X\n", g_aCommands[ CMD_BOOKMARK_ADD ].m_sName, iBookmark, g_aBookmarks[ iBookmark ].nAddress);
		}
		iBookmark++;
	}
...
Update_t CmdBreakOpcode (int nArgs)  // Breakpoint IFF Full-speed!
{
	...
	if (g_iDebugBreakOnOpcode == 0)
		...
		ConsoleBufferPushFormat("%s Break on Opcode: None", sAction);
	else
		// Show what the current break opcode is
		ConsoleBufferPushFormat("%s Break on Opcode: %02X %s", sAction, g_iDebugBreakOnOpcode, g_aOpcodes65C02[ g_iDebugBreakOnOpcode ].sMnemonic);
...
Update_t CmdBreakOnInterrupt (int nArgs)
{
	...
	ConsoleBufferPushFormat("%s Break on Interrupt: %s", sAction, g_bDebugBreakOnInterrupt ? "Enabled" : "Disabled");
...
Update_t CmdBreakpointAddReg (int nArgs)
{
	...
	while (iArg++ < nArgs)
	{
		...
		if (nFound)
		{
			switch (iParamCmp)
			{
				case PARAM_BP_LESS_EQUAL:
					iCmp     = BP_OP_LESS_EQUAL;
					bHaveCmp = true;
					break;
				case PARAM_BP_LESS_THAN:
					iCmp     = BP_OP_LESS_THAN;
					bHaveCmp = true;
					break;
				case PARAM_BP_EQUAL:
					iCmp     = BP_OP_EQUAL;
					bHaveCmp = true;
					break;
				case PARAM_BP_NOT_EQUAL:
					iCmp     = BP_OP_NOT_EQUAL;
					bHaveCmp = true;
					break;
				case PARAM_BP_NOT_EQUAL_1:
					iCmp     = BP_OP_NOT_EQUAL;
					bHaveCmp = true;
					break;
				case PARAM_BP_GREATER_THAN:
					iCmp     = BP_OP_GREATER_THAN;
					bHaveCmp = true;
					break;
				case PARAM_BP_GREATER_EQUAL:
					iCmp     = BP_OP_GREATER_EQUAL;
					bHaveCmp = true;
					break;
				default:
					break;
...
Update_t CmdBreakpointSave (int nArgs)
{
	...
	while (iBreakpoint < MAX_BREAKPOINTS)
	{
		...
		{
			g_ConfigState.PushLineFormat("%s %x %04X,%04X\n", g_aCommands[ CMD_BREAKPOINT_ADD_REG ].m_sName, iBreakpoint, g_aBreakpoints[ iBreakpoint ].nAddress, g_aBreakpoints[ iBreakpoint ].nLength);
		}
		if (! g_aBreakpoints[ iBreakpoint ].bEnabled)
		{
			g_ConfigState.PushLineFormat("%s %x\n", g_aCommands[ CMD_BREAKPOINT_DISABLE ].m_sName, iBreakpoint);
		}
...
void ConfigSave_PrepareHeader (const Parameters_e eCategory, const Commands_e eCommandClear)
{
	g_ConfigState.PushLineFormat("%s %s = %s\n", g_aTokens[ TOKEN_COMMENT_EOL ].sToken, g_aParameters[ PARAM_CATEGORY ].m_sName, g_aParameters[ eCategory ].m_sName);

	g_ConfigState.PushLineFormat("%s %s\n", g_aCommands[ eCommandClear ].m_sName, g_aParameters[ PARAM_WILDSTAR ].m_sName);
}
...
Update_t CmdConfigDisasm (int nArgs)
{
	...
	for (int iArg = 1; iArg <= nArgs; iArg++)
	{
		...
		else if (FindParam(g_aArgs[ iArg ].sArg, MATCH_FUZZY, iParam))
		{
		}
...
#define DEBUG_SCROLL 3
		...
		do
		{
			...
			if ((iOpmode == AM_1) || (iOpmode == AM_2) || (iOpmode == AM_3) || ! _6502_IsOpcodeValid(iOpcode) || (nOpbytes != 1))
			{
				g_nDisasmTopAddress--;
				DisasmCalcCurFromTopAddress();
				...
				if ((iOpmode == AM_1) || (iOpmode == AM_2) || (iOpmode == AM_3) || ! _6502_IsOpcodeValid(iOpcode) || (nOpbytes != 2))
				{
					g_nDisasmTopAddress--;
					DisasmCalcCurFromTopAddress();
					...
					if ((iOpmode == AM_1) || (iOpmode == AM_2) || (iOpmode == AM_3) || (nOpbytes != 3))
						g_nDisasmTopAddress--;
					DisasmCalcCurFromTopAddress();
				}
			}
		...
		if (nCandidates)
		{
			...
			for (; iCandidate < nCandidates; iCandidate++)
			{
				...
				if ((iOpmode != AM_1) && (iOpmode != AM_2) && (iOpmode != AM_3) && _6502_IsOpcodeValid(iOpcode))
				{
					...
					else if (g_iConfigDisasmScroll == 3)
					{
						// Favor max opbytes
						iBest = iCandidate;
					}
...
Update_t CmdDisk (int nArgs)
{
	...
	else if (iParam == PARAM_DISK_PROTECT)
	{
		if (nArgs > 3)
			return HelpLastCommand();
...
Update_t CmdConfigSetDebugDir (int nArgs)
{
	...
	else  // Relative
	{
		...
		while ((iPathSeparator = sNewPath.find(PATH_SEPARATOR, iPrevSeparator)) != std::string::npos)
		{
			...
			else if ((nSubDirLen == 3) && (sSubDir == UP_DIR))  // Up directory "..\" in the subpath?
			{
		...
		else if (iParam == PARAM_LOAD)
		{
			...
			if (pFile)
			{
				...
				if (iFileType == TYPE_BMP)
				{
					...
					if (! (((bmp.nWidthPixels == 64) && (bmp.nHeightPixels == 256)) || ((bmp.nWidthPixels == 64) && (bmp.nHeightPixels == 1)) || ((bmp.nWidthPixels == 16) && (bmp.nHeightPixels == 1))))
					{
					...
					else  // 0x7C version4 bitmap
					{
						if (bmp.nCompression == 3)  // BI_BITFIELDS
						{
							if ((bmp.nRedMask == 0xFF000000)  // Gimp writes in ABGR order
							    && (bmp.nGreenMask == 0x00FF0000) && (bmp.nBlueMask == 0x0000FF00))
								bSwizzle = true;
						}
					}
				}
...
Update_t CmdOutputPrintf (int nArgs)
{
	...
	for (int iArg = 1; iArg <= nArgs; iArg++)
	{
		if (g_aArgs[ iArg ].bType & TYPE_QUOTED_2)
			continue;
		else if (g_aArgs[ iArg ].eToken == TOKEN_COMMA)
			continue;
		else
		{
			Arg_t entry;
			...
			else
			{
				...
				if (BufferAssemblyListing(sFileName))
				{
					...
					else
					{
						...
						{
							ConsoleBufferPushFormat("  Read: %d lines, %d symbols, %d bytes", g_AssemblerSourceBuffer.GetNumLines()  // g_nSourceAssemblyLines
							                        ,
							                        g_nSourceAssemblySymbols, g_nSourceAssembleBytes);
						}
						else
						{
							ConsoleBufferPushFormat("  Read: %d lines, %d symbols", g_AssemblerSourceBuffer.GetNumLines()  // g_nSourceAssemblyLines
							                        ,
							                        g_nSourceAssemblySymbols);
						}
					}
...
{
	ConsolePrintFormat("Ambiguous " CHC_NUM_DEC "%" SIZE_T_FMT CHC_DEFAULT " Commands:", g_vPotentialCommands.size());
...
void ProfileFormat (bool bExport, ProfileFormat_e eFormatMode)
{
	...
	else if (eFormatMode == PROFILE_FORMAT_SPACE)
	{
	...
	for (int iOpcode = 0; iOpcode < NUM_OPCODES; ++iOpcode)
	{
		...
		prfline.Format("%s%7.4f%s%%" DELIM "%s%9u" DELIM "%s%s" DELIM "%s%s" DELIM "%s%s\n", pColorNumber, nPercent, pColorOperator, sSeparator2.c_str(), pColorNumber, static_cast<unsigned int>(nCount), sSeparator2.c_str(), pColorOpcode, sOpcode.c_str(), sSeparator2.c_str(), pColorMnemonic, g_aOpcodes[ nOpcode ].sMnemonic, sSeparator2.c_str(), pColorOpmode, sAddress.c_str());
		prfline = ProfileLinePush();
	}
	...
	prfline.Format("Total:  " DELIM "%s%9u\n", sSeparator2.c_str(), pColorTotal, static_cast<unsigned int>(nOpcodeTotal));
	prfline = ProfileLinePush();
	...
	for (int iOpmode = 0; iOpmode < NUM_OPMODES; ++iOpmode)
	{
		...
		// BUG: Yeah 100% is off by 1 char. Profiling only one opcode isn't worth fixing this visual alignment bug.
		prfline.Format("%s%7.4f%s%%" DELIM "%s%9u" DELIM "%s%s\n", pColorNumber, nPercent, pColorOperator, sSeparator2.c_str(), pColorNumber, static_cast<unsigned int>(nCount), sSeparator2.c_str(), pColorOpmode, sAddress.c_str());
		prfline = ProfileLinePush();
	}
	...
	prfline.Format("Total:  " DELIM "%s%9u\n", sSeparator2.c_str(), pColorTotal, static_cast<unsigned int>(nOpmodeTotal));
	prfline = ProfileLinePush();
	...
	prfline.Format("Cycles: " DELIM "%s%9u\n", sSeparator2.c_str(), pColorNumber, cycles);
	prfline = ProfileLinePush();
}
...
void DebuggerInputConsoleChar (TCHAR ch)
{
	...
	else if (ch == 0x16)  // HACK: Ctrl-V.  WTF!?
	{
		...
		else
		{
			switch (keycode)
			{
				case VK_TAB:
				{
					...
					else if (KeybGetCtrlStatus() && KeybGetShiftStatus())
				...
				case VK_HOME:
					...
					else if (g_nConsoleInputChars > 0)
					{
						if (KeybGetShiftStatus())
						{
