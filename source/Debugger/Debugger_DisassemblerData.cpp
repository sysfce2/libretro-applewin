...
Update_t CmdDisasmDataDefCode (int nArgs)
{
	...
	*/
	while (nAddress <= tData.nEndAddress)
	{
		...
		if (pData)
		{
			if ((nAddress <= pData->nStartAddress) && (tData.nEndAddress >= pData->nEndAddress))
			{
				// remove entire
				Disassembly_DelData(*pData);
			}
			else if ((nAddress <= pData->nStartAddress) && (tData.nStartAddress < pData->nEndAddress))
			{
				// head
				pData->nStartAddress = nAddress + 1;
			}
			else if ((nAddress > pData->nStartAddress) && (tData.nEndAddress >= pData->nEndAddress))
			{
				// tail
...
const char *g_aNopcodeTypes[ NUM_NOPCODE_TYPES ] =
    {
        "-n/a-", "byte1", "byte2", "byte4", "byte8", "word1", "word2", "word4", "addr ", "hex  ", "char ", "ascii", "apple", "mixed", "FAC  ", "bmp  "};
...
Update_t CmdDisasmDataList (int nArgs)
{
	...
	while ((pData = Disassembly_Enumerate(pData)))
	{
		if (pData->iDirective != _NOP_REMOVED)
		{
			...
			// `TEST `300`:`320
			ConsolePrintFormat(CHC_CATEGORY "%s %s%*s " CHC_ADDRESS "%04X" CHC_ARG_SEP ":" CHC_ADDRESS "%04X", g_aNopcodeTypes[ pData->eElementType ], hasSymbol ? CHC_SYMBOL : CHC_DEFAULT, MAX_SYMBOLS_LEN, hasSymbol ? pData->sSymbol : "???", pData->nStartAddress, pData->nEndAddress  // Disassembly_IsDataAddress() is *inclusive* // KEEP IN SYNC:  _CmdDefineByteRange() CmdDisasmDataList() _6502_GetOpmodeOpbyte() FormatNopcodeBytes()
			);
		}
	}
