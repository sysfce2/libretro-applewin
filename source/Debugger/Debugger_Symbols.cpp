...
const char *g_sFileNameSymbols[ NUM_SYMBOL_TABLES ] = {
    "APPLE2E.SYM", "A2_BASIC.SYM", "A2_ASM.SYM", "A2_USER1.SYM"  // "A2_USER.SYM",
    ,
    "A2_USER2.SYM", "A2_SRC1.SYM"  // "A2_SRC.SYM",
    ,
    "A2_SRC2.SYM", "A2_DOS33.SYM", "A2_PRODOS.SYM"};
std::string g_sFileNameSymbolsUser;

const char *g_aSymbolTableNames[ NUM_SYMBOL_TABLES ] =
    {
        "Main", "Basic", "Asm"  // "Assembly",
        ,
        "User1"  // User
        ,
        "User2", "Src1", "Src2", "DOS33", "ProDOS"};
...
Update_t _PrintSymbolInvalidTable ()
{
	...
	ConsolePrintFormat("Only " CHC_NUM_DEC "%d" CHC_DEFAULT " symbol tables are supported:", NUM_SYMBOL_TABLES);
	...
	{
		sText += StrFormat(CHC_USAGE "%s" CHC_ARG_SEP "%c ", g_aSymbolTableNames[ iTable ], (iTable != (NUM_SYMBOL_TABLES - 1)) ? ',' : '.');
	}
...
std::string _CmdSymbolsInfoHeader (int iTable, int nDisplaySize /* = 0 */)
{
	...
	return StrFormat(CHC_USAGE "%s" CHC_ARG_SEP ":%s%d "  // CHC_DEFAULT
	                 ,
	                 g_aSymbolTableNames[ iTable ], bActive ? CHC_NUM_DEC : CHC_WARNING, nSymbols);
}
...
void _CmdPrintSymbol (LPCTSTR pSymbol, WORD nAddress, int iTable)
{
	...
	ConsolePrintFormat("  %s%s%s: $%s%04X %s%s", CHC_USAGE, g_aSymbolTableNames[ iTable ], CHC_ARG_SEP, CHC_ADDRESS, nAddress, CHC_SYMBOL, pSymbol);
	...
	for (int iArgs = 1; iArgs <= nArgs; iArgs++)
	{
		...
		{  // Have address, do symbol lookup first
			if (! _CmdSymbolList_Symbol2Address(pSymbol, bSymbolTables))
			{
				...
				{
					ConsolePrintFormat(" Address not found: %s$%s%04X%s", CHC_ARG_SEP, CHC_ADDRESS, nAddress, CHC_DEFAULT);
				}
			}
		}
		else
		{  // Have symbol, do address lookup
			...
			{  // nope, ok, try as address
				if (String2Address(pSymbol, nAddress))
				{
					if (! _CmdSymbolList_Address2Symbol(nAddress, bSymbolTables))
					{
						ConsolePrintFormat(" %sSymbol not found: %s%s%s", CHC_ERROR, CHC_SYMBOL, pSymbol, CHC_DEFAULT);
					}
				}
				else
				{
...
int ParseSymbolTable (const std::string &pPathFileName, SymbolTable_Index_e eSymbolTableWrite, int nSymbolOffset)
{
	...
	if (hFile)
	{
		while (! feof(hFile))
		{
			...
			if (nLen > nMaxLen)
			{
				ConsolePrintFormat(" %sWarn.: %s%s %s(%s%d %s> %s%d%s)", CHC_WARNING, CHC_SYMBOL, sName, CHC_ARG_SEP, CHC_NUM_DEC, nLen, CHC_ARG_SEP, CHC_NUM_DEC, nMaxLen, CHC_ARG_SEP);
			...
			if (pSymbolPrev)
			{
				if (! bFileDisplayed)
				{
					...
					ConsolePrintFormat("%s%s", CHC_PATH, pPathFileName.c_str());
				}
				...
				ConsolePrintFormat(" %sInfo.: %s%-16s %saliases %s$%s%04X %s%-12s%s (%s%s%s)"  // MAGIC NUMBER: -MAX_SYMBOLS_LEN
				                   ...
				                   ,
				                   CHC_SYMBOL, sName, CHC_INFO, CHC_ARG_SEP, CHC_ADDRESS, nAddress, CHC_SYMBOL, pSymbolPrev->c_str(), CHC_DEFAULT, CHC_STRING, g_aSymbolTableNames[ iTable ], CHC_DEFAULT);
			...
			if (bExists)
			{
				if (! bDupSymbolHeader)
				{
					...
					ConsolePrintFormat(" %sDup Symbol Name%s (%s%s%s) %s", CHC_ERROR, CHC_DEFAULT, CHC_STRING, g_aSymbolTableNames[ iTable ], CHC_DEFAULT, pPathFileName.c_str());
				}

				ConsolePrintFormat("  %s$%s%04X %s%-31s%s", CHC_ARG_SEP, CHC_ADDRESS, nAddress, CHC_SYMBOL, sName, CHC_DEFAULT);
			}
	...
	if (_tcslen(pSymbolName) < MAX_SYMBOLS_LEN)
	{
		...
		if (bExists)
		{
			if (iTable == eSymbolTable)
			{
				...
				if (bUpdateSymbol)
				{
					ConsolePrintFormat(" Updating %s%s%s from %s$%s%04X%s to %s$%s%04X%s", CHC_SYMBOL, pSymbolName, CHC_DEFAULT, CHC_ARG_SEP, CHC_ADDRESS, nAddressPrev, CHC_DEFAULT, CHC_ARG_SEP, CHC_ADDRESS, nAddress, CHC_DEFAULT);
				}
			}
		}
		else
		...
		if (bUpdateSymbol)
		{
			...
			ConsolePrintFormat(" Added symbol: %s%s%s %s$%s%04X%s", CHC_SYMBOL, pSymbolName, CHC_DEFAULT, CHC_ARG_SEP, CHC_ADDRESS, nAddress, CHC_DEFAULT);
		}
	}
}
...
Update_t _CmdSymbolsUpdate (int nArgs, int bSymbolTables)
{
	...
	if ((nArgs == 2) && ((g_aArgs[ 1 ].eToken == TOKEN_EXCLAMATION) || (g_aArgs[ 1 ].eToken == TOKEN_TILDE)))
		bRemoveSymbol = true;
	...
	if ((nArgs == 2) && (g_aArgRaw[ 1 ].eToken == TOKEN_AT)  // NOTE: @ is parsed and evaluated and NOT in the cooked args
		...
		if (nParams)
		{
			if (iParam == PARAM_CLEAR)
			{
				...
				if (iTable != NUM_SYMBOL_TABLES)
				{
					...
					ConsolePrintFormat(" Cleared symbol table: %s%s", CHC_STRING, g_aSymbolTableNames[ iTable ]);
