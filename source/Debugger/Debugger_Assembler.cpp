...
int _6502_GetOpmodeOpbyte (const int nBaseAddress, int &iOpmode_, int &nOpbyte_, const DisasmData_t **pData_)
{
	...
	if (pData)
	{
		...
		switch (pData->eElementType)
		{
			...
			case NOP_ADDRESS:
				nOpbyte_              = 2;
				iOpmode_              = AM_A;  // BUGFIX: 2.6.2.33 Define Address should be shown as Absolute mode, not Indirect Absolute mode. DA BASIC.FPTR D000:D080 // was showing as "da (END-1)" now shows as "da END-1"
...
void _CmdAssembleHashDump ()
{
	...
	for (iOpcode = 0; iOpcode < NUM_OPCODES; iOpcode++)
	{
		...
		ConsoleBufferPushFormat("%08X %02X %s %s", iThisHash, nOpcode, g_aOpcodes65C02[ nOpcode ].sMnemonic, g_aOpmodes[ nOpmode ].m_sName);
		nThisHash++;
