...
static void PrintGlyph (const int xDst, const int yDst, const int glyph)
{
	...
	//
	{
		...
		if ((col < DEBUG_VIRTUAL_TEXT_WIDTH) && (row < DEBUG_VIRTUAL_TEXT_HEIGHT))
                                        ...
                                        const int iCtrlBackground = BG_INFO_CHAR, const int iCtrlForeground = FG_INFO_CHAR_LO)
{
	...
	//	if (hDC)
	{
		SetupColorsHiLoBits(bHighBit, bCtrlBit, iTextBG, iTextFG  // FG_DISASM_CHAR
		                    ,
...
WORD DrawDisassemblyLine (int iLine, const WORD nBaseAddress)
{
	...
	if (((! g_bDisasmCurBad) && (iLine == g_nDisasmCurLine)) || (g_bDisasmCurBad && (iLine == 0)))
	{
		bCursorLine = true;
...
void _DrawSoftSwitchLanguageCardBank (RECT &rect, const int iBankDisplay, int bg_default = BG_INFO)
{
	...
	else
	{
		...
		{
			cMemType    = 's';
			iActiveBank = GetCardMgr().GetLanguageCard()->GetActiveBank();
		}
...
void DrawZeroPagePointers (int line)
{
	...
	for (int iZP = 0; iZP < MAX_ZEROPAGE_POINTERS; iZP++)
	{
		...
		if (bEnabled)
		{
			...
			else if (! sSymbol2.empty() && sSymbol2[ 0 ] == '$')
			{
				//				sText = pSymbol2;
...
void UpdateDisplay (Update_t bUpdate)
{
	...
	if ((bUpdate & UPDATE_BREAKPOINTS)
	    ...
	    || (bUpdate & UPDATE_FLAGS) || (bUpdate & UPDATE_MEM_DUMP) || (bUpdate & UPDATE_REGS) || (bUpdate & UPDATE_STACK) || (bUpdate & UPDATE_SYMBOLS) || (bUpdate & UPDATE_TARGETS) || (bUpdate & UPDATE_WATCH) || (bUpdate & UPDATE_ZERO_PAGE))
	{
		bUpdate |= UPDATE_BACKGROUND;
