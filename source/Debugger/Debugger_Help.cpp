...
void Help_Categories ()
{
	...
	ConsolePrintFormat(CHC_USAGE "Notes" CHC_DEFAULT ": " CHC_ARG_MAND "<>" CHC_DEFAULT " = mandatory, " CHC_ARG_OPT "[]" CHC_DEFAULT " = optional, " CHC_ARG_SEP "|" CHC_DEFAULT " argument option");
	//	ConsoleBufferPush( sText.c_str() );
}

void Help_Examples ()
{
	ConsolePrintFormat(" %sExamples%s:%s", CHC_USAGE, CHC_ARG_SEP, CHC_DEFAULT);
}
...
bool Colorize (char *pDst, size_t /*nDstSz*/, const char *pSrc)
{
	...
	while (*pSrc)
	{
		...
		else if (strncmp(sSeeAlso, pSrc, nSeeAlso) == 0)
		{
			_ColorizeHeader(pDst, pSrc, sSeeAlso, nSeeAlso);
		}
		else if (strncmp(sNote, pSrc, nNote) == 0)
		{
			_ColorizeHeader(pDst, pSrc, sNote, nNote);
		}
		else if (strncmp(sTotal, pSrc, nNote) == 0)
		{
			_ColorizeHeader(pDst, pSrc, sTotal, nTotal);
		}
		else if (strncmp(sExamples, pSrc, nExamples) == 0)
		{
		...
		else if ((*pSrc == '$') && isHexDigit(pSrc[ 1 ]))  // Hex Number
		{
			_ColorizeOperator(pDst, pSrc, CHC_ARG_SEP);
...
Update_t CmdMOTD (int nArgs)  // Message Of The Day
{
	...
	CmdVersion(0);
	CmdSymbols(0);
	ConsoleColorizePrintFormat("  '%sCtrl ~'%s console, '%s%s'%s (specific), '%s%s'%s (all)", CHC_KEY, CHC_DEFAULT, CHC_COMMAND, g_aCommands[ CMD_HELP_SPECIFIC ].m_sName, CHC_DEFAULT
	                           //		, g_aCommands[ CMD_HELP_SPECIFIC ].pHelpSummary
	                           ,
	                           CHC_COMMAND, g_aCommands[ CMD_HELP_LIST ].m_sName, CHC_DEFAULT
...
Update_t CmdHelpSpecific (int nArgs)
{
	...
	for (iArg = 1; iArg <= nArgs; iArg++)
	{
		...
		if (nFound && (! bAllCommands) && bDisplayCategory)
		{
			...
			ConsolePrintFormat("%sCategory%s: %s%s", CHC_USAGE, CHC_DEFAULT, CHC_CATEGORY, pCategory);
		...
		switch (iCommand)
		{
			...
			case CMD_OUT:
				...
				break;
			case CMD_PROFILE:
				ConsoleColorizePrintFormat(" Usage: [%s | %s | %s]", g_aParameters[ PARAM_RESET ].m_sName, g_aParameters[ PARAM_SAVE ].m_sName, g_aParameters[ PARAM_LIST ].m_sName);
			...
			case CMD_REGISTER_SET:
				...
				ConsoleColorizePrintFormat(" See also: %s%s", CHC_CATEGORY, g_aParameters[ PARAM_CAT_OPERATORS ].m_sName);
				Help_Examples();
			...
			case CMD_BREAK_INVALID:
				ConsoleColorizePrintFormat(" Usage: [%s%s | %s%s]", CHC_COMMAND, g_aParameters[ PARAM_ON ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_OFF ].m_sName);
				ConsoleColorizePrintFormat(" Usage: [# | # %s%s | # %s%s]", CHC_COMMAND, g_aParameters[ PARAM_ON ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_OFF ].m_sName);
				ConsoleColorizePrintFormat(" Usage: [%s%s %s%s | %s%s %s%s]", CHC_COMMAND, g_aParameters[ PARAM_ALL ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_ON ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_ALL ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_OFF ].m_sName);
				ConsoleColorizePrint("Where: # is 0=BRK, 1=Invalid Opcode_1, 2=Invalid Opcode_2, 3=Invalid Opcode_3");
				break;
				//		case CMD_BREAK_OPCODE:
			case CMD_BREAKPOINT:
				ConsoleColorizePrintFormat(" Usage: [%s%s | %s%s | %s%s]", CHC_COMMAND, g_aParameters[ PARAM_LOAD ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_SAVE ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_RESET ].m_sName);
			...
			case CMD_CONFIG_DISASM:
			{
				...
				ConsoleColorizePrintFormat(" Usage: [%s%s | %s%s | %s%s | %s%s | %s%s | %s%s]", CHC_COMMAND, g_aParameters[ PARAM_CONFIG_BRANCH ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_CONFIG_CLICK ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_CONFIG_COLON ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_CONFIG_OPCODE ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_CONFIG_SPACES ].m_sName, CHC_COMMAND, g_aParameters[ PARAM_CONFIG_TARGET ].m_sName);
			...
			case CMD_SYMBOLS_LOOKUP:
				ConsoleColorizePrint(" Usage: symbol [= <address>]");
				ConsolePrintFormat("       %s\"%ssymbol%s\" = %saddress", CHC_ARG_MAND, CHC_SYMBOL, CHC_ARG_MAND, CHC_ADDRESS);
				ConsoleColorizePrint(" Note: Valid characters are any characters above ASCII space ($20).");
				ConsolePrintFormat(" You %sMUST%s double-quote names containing special characters to be recognized.", CHC_WARNING, CHC_DEFAULT);
				Help_Examples();
			...
			case CMD_VIDEO_SCANNER_INFO:
				...
				ConsolePrintFormat("    %sYellow%s=invisible (hbl or vbl active) / %sGreen%s=visible", CHC_INFO, CHC_DEFAULT  // yellow
				                   ,
...
Update_t CmdVersion (int nArgs)
{
	...
	ConsolePrintFormat("  Emulator:  %s%s%s    Debugger: %s%d.%d.%d.%d%s", CHC_SYMBOL, g_VERSIONSTRING.c_str(), CHC_DEFAULT, CHC_SYMBOL, nMajor, nMinor, nFixMajor, nFixMinor, CHC_DEFAULT);
		...
		for (int iArg = 1; iArg <= g_nArgRaw; iArg++)
		{
			    ...
			    (! _tcscmp(g_aArgs[ iArg ].sArg, g_aParameters[ PARAM_MEM_SEARCH_WILD ].m_sName)))
			{
				...
				ConsoleBufferPushFormat("  Cursor(%d)  T: %04X  C: %04X  B: %04X %c D: %02X",  // Top, Cur, Bot, Delta
				                        ...
				                        g_bDisasmCurBad ? TEXT('*') : TEXT(' '), g_nDisasmBotAddress - g_nDisasmTopAddress);
